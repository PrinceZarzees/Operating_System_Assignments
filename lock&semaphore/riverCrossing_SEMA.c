...
void* hackerArrives(void* data) {
    ...
    printf("||(%d) hacker reacher on the other side||\n", id);
}   
...
void* serfArrives(void* data) {
    ...
    printf("||(%d) serf reached on the other side||\n", id);
}
...
int main(int argc, char* argv[]) {
    ...
    for(int i=0;i<NUM_HACKER;i++) {
        ...
        // printf("||hacker (%d) reached on the other side||\n", hackerID[i]);
    }

    for(int i=0;i<NUM_SERF;i++) {
        pthread_join(serf[i], NULL);
        // printf("||serf (%d) reached on the other side||\n", serfID[i]);
    }
